...
<html lang="en">
  ...
  <body>
      ...
      <div class="row">
        ...
        <nav id="sidebar" class="sidebar">
          <div class="d-flex flex-column align-items-center position-sticky">
            ...
            <!-- Tombol Logout -->
            <button class="btn btn-warning text-black mt-4" style="background-color: #ffda00"><i class="fa-solid fa-right-from-bracket fa-rotate-180 mx-2"></i>Logout</button>
          </div>
        </nav>
        ...
        <div class="container-fluid" style="display: flex">
          <main class="main absolute">
            <div class="d-flex justify-content-between align-items-center mb-3">
              ...
              <!-- tambah sekre -->
              <button class="btn btn-primary tambah-button" data-toggle="modal" data-target="#tambahUnitModal"><i class="fas fa-plus"></i> Tambah</button>
            </div>
            <!-- Tabel Unit  -->
            <table id="tabelUnit" class="table table-striped table-bordered" style="width: 100%">
              <thead>
                <tr>
                  <th>No</th>
                  <th>Unit</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <button class="btn btn-edit" data-toggle="modal" data-target="#editUnitModal{{$unit->id}}">Edit</button>
                    <button class="btn btn-hapus" onclick="hapusUnit({{$unit->id}})">Hapus</button>
                  </td>
                </tr>
                @endforeach
                <!-- Batas Tabel-->
            ...
            @foreach ($units as $unit)
            <form id="deleteForm" action="{{ route('unit.destroy', ['id' => $unit->id]) }}"method="POST">
              @method('DELETE')
              @csrf
            <div id="confirmModal" class="confirm-modal">
              <div class="confirm-box">
                <p>Apakah Anda yakin ingin menghapus Data Unit ini?</p>
                <button class="ok float-right" onclick="deleteUnit()">Ya</button>
                <button class="cancel" onclick="hideConfirmModal()">Tidak</button>
              </div>
            </div>
        </form>
        @endforeach
          ...
          <!-- Modal Tambah Unit-->
          <div class="modal" tabindex="-1" role="dialog" id="tambahUnitModal">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                  <!--  form untuk menambahkan data unit baru -->
                  <form action="{{ route('superadmin.tambahUnit') }}" method="POST">
                    @csrf
                    <div class="form-group">
                      <label for="unit">Unit</label>
                      <input type="text" class="form-control" id="unit" name='nama_unit' required />
                    </div>
                  
                </div>
                <div class="modal-footer">
                  <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
                  <button type="submit" class="btn btn-primary" onclick="simpanUnit()">Simpan</button>
                </div>
            </form>
              </div>
            </div>
          ...
          <!-- Modal Edit Unit -->
          @foreach ($units as $unit)
          <div class="modal" tabindex="-1" role="dialog" id="editUnitModal{{$unit->id}}">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                ...
                <!-- form untuk mengubah data peserta -->
            
                <div class="modal-body">
                    <form action="{{ route('update-dataunit', ['id' => $unit->id]) }}"method="POST">
                        @method('PUT')
                        @csrf
                    <div class="form-group">
                      <label for="unit">Unit</label>
                      <input type="text" class="form-control" id="unit" name="nama_unit" value="{{$unit->nama_unit}}"required />
                    </div>
    ...
    <script>
      ...
      // Menggunakan modifikasi fungsi hapus Unit
      ...
      function hapusUnitKonfirmasi(confirmation) {
        // Sembunyikan modal konfirmasi hapus
        hideConfirmModal();

        // Jika dikonfirmasi (OK), hapus data Peserta
        if (confirmation) {
          console.log("Hapus Unit dengan ID: " + idToDelete);
        } 
      }
    </script>
    ...
    <script>
      document.addEventListener("DOMContentLoaded", function () {
        ...
        function showTambahUnitModal() {
          $("#tambahUnitModal").modal("show");
        }

        // Fungsi untuk menyembunyikan modal
        function hideTambahUnitModal() {
          $("#tambahUnitModal").modal("hide");
        }

        // event listener ke tombol "Tambah"
        document.querySelector(".tambah-button").addEventListener("click", showTambahUnitModal);

        // event listener ke tombol "Simpan" di dalam modal
        document.querySelector("#tambahUnitModal .btn-primary").addEventListener("click", function () {
    ...
    <script>
      document.addEventListener("DOMContentLoaded", function () {
        ...
        document.querySelector(".btn-edit").addEventListener("click", showEditUnitModal);

        // event listener ke tombol "Simpan" di dalam modal
        document.querySelector("#EditUnitModal .btn-primary").addEventListener("click", function () {
          console.log("Simpan data peserta");
