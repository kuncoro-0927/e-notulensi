        ...
        <nav id="sidebar" class="sidebar">
          <div class="d-flex flex-column align-items-center position-sticky">
            ...
            <ul class="nav flex-column mt-4">
              ...
              <li class="nav-item">
                <a id="dashboardLink" class="nav-link active text-white" href="{{ url('/sekretaris/dashboard') }}" > <i class="fas fa-home mx-2"></i> Dashboard </a>
              </li>
              <li class="nav-item">
                <a id="buatNotulensiLink" class="nav-link active text-white" href="{{ url('/sekretaris/buat_notula') }}"><i class="fas fa-pen mx-2"></i> Buat Notulensi </a>
              </li>
              <li class="nav-item">
        ...
        <main class="main absolute">
            <div class="profile-container bg-white p-3 mb-4" style="box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                <div class="d-flex align-items-center">
                  <div>
                    <h5 class="m-0 text-end">Upload Document</h5>
                  </div>
                </div>
              </div>
...
@endif
        ...
        @csrf
        <input type="hidden" name="notula_id" value="{{ $items->id }}">
        <div class="form-group">
          <label for="undangan[]">Undangan</label>
          <input type="file" class="form-control-file" id="undangan"  name="undangan[]" multiple  accept=".pdf, .docx, .jpg, .png, .doc"/>
        </div>
        <div class="form-group">
          <label for="presensi[]">Presensi</label>
          <input type="file" class="form-control-file" id="presensi"  name="presensi[]" multiple accept=".pdf, .docx, .jpg, .png, .doc" />
        </div>
        <div class="form-group">
          <label for="dokumentasi[]">Dokumentasi</label>
          <input type="file" class="form-control-file" id="dokumentasi" name="dokumentasi[]" multiple accept=".pdf, .docx, .jpg, .png, .doc" />
        </div>
      

          <div class="d-flex justify-content-center mt-3">
            <button type="button" class="btn btn-secondary mr-2" onclick="goToDashboard()">Kembali</button>
...
function Logout() {
    ...
    window.location.href = "login";
      ...
      function goToDashboard() {
        console.log("Function called");
        window.location.href = "/sekretaris/dashboard";
      }
